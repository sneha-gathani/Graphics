...
public:
    ...
    // bool traversal(const Ray3f &ray_, Intersection &its, bool shadowRay, Node *node, bool &foundIntersection, uint32_t &f) const{
    //     return m_accel->traversal(ray_, its, false, node, foundIntersection, f);
    // }
    ...
    std::vector<Mesh *> getEmittermeshes() const{
        return m_emitterMeshes;
    }
...
private:
    ...
    std::vector<Mesh *> m_emitterMeshes;
};

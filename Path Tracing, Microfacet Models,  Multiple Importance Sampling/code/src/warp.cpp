...
Vector3f Warp::squareToBeckmann(const Point2f &sample, float alpha) {
    float theta = std::atan(std::sqrtf(-alpha * alpha * std::log(1.0f - sample.y())));
    float phi = 2 * M_PI * sample.x();
    float z = cos(theta);
    float x = sin(theta) * cosf(phi);
    float y = sin(theta) * sinf(phi);
    return Vector3f(x, y, z);
}

float Warp::squareToBeckmannPdf(const Vector3f &m, float alpha) {
    float numerator = std::expf((-Frame::tanTheta(m) * Frame::tanTheta(m)) / (alpha * alpha));
    float denominator = M_PI * alpha * alpha * std::pow(Frame::cosTheta(m), 3);
    
    if(std::isinf(Frame::tanTheta(m) * Frame::tanTheta(m)))
        return 0.0f; 

    if(Frame::cosTheta(m) < 0.0f)
        return 0.0f;

    if (m.z() > 0.f && m.norm() <= (1.f + Epsilon))
